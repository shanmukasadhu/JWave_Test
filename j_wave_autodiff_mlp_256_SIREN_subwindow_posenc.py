"""
SIREN material field with sinusoidal position encodings on input (x, y).
Based on j_wave_autodiff_mlp_256_SIREN_subwindow.py but adds γ(x), γ(y) encoding
(NeRF-style: sin/cos at 2^0, 2^1, ..., 2^(L-1) frequencies) before the SIREN.
"""
import numpy as np
...
results_dir = os.path.join('Results/autodiff_mlp_256_SIREN_subwindow_posenc', _run_id) if _run_id else 'Results/autodiff_mlp_256_SIREN_subwindow_posenc'
os.makedirs('Results/autodiff_mlp_256_SIREN_subwindow_posenc', exist_ok=True)
os.makedirs(results_dir, exist_ok=True)

print("="*70)
print("SIREN + POSITION ENCODING — SUBWINDOW (75% OF DESIGN SPACE)")
print("="*70)
print("\n256×256 grid. Sinusoidal pos encodings on (x,y) before SIREN.")
print("Material is learned ONLY inside a subwindow (75% of design space).")
print("Everything outside the subwindow is water.")
print("="*70)
...
target_focal_center = (160, 128)
focal_disk_radius = 5  # grid points
...
# Position encoding: number of frequency bands (NeRF-style)
pos_enc_num_frequencies = 4   # 2^0, 2^1, ..., 2^(L-1) → input dim = 2 + 4*L = 18

# SIREN architecture: encoded (x,y) -> [0,1] material
# Input dim = 2 (raw x,y) + 2*pos_enc_num_frequencies*2 (sin/cos for x and y) = 2 + 8*L
siren_input_dim = 2 + 2 * pos_enc_num_frequencies * 2  # 2 + 4*4 = 18 for L=4
...
print(f"  Position encoding: {pos_enc_num_frequencies} frequency bands (input dim {siren_input_dim})")
print(f"  SIREN: {siren_layer_sizes}, ω₀={siren_omega0}")
print(f"  Subwindow: x=[{subwindow_x_start},{subwindow_x_end}], y=[{subwindow_y_start},{subwindow_y_end}]")
print(f"  Objective: maximize RMS pressure over focal disk")
print("="*70)

# ============================================================================
# Position encoding: γ(p) = [sin(2^0 π p), cos(2^0 π p), ..., sin(2^(L-1) π p), cos(2^(L-1) π p)]
# For (x,y): concat([x, y, γ(x), γ(y)])
# ============================================================================

def positional_encode_2d(xy, num_frequencies):
    """
    Encode 2D coords (x,y) with sinusoidal position encodings.
    xy: (2,) or (N, 2). Returns (siren_input_dim,) or (N, siren_input_dim).
    """
    x, y = xy[..., 0], xy[..., 1]
    freqs = 2.0 ** jnp.arange(num_frequencies)  # 2^0, 2^1, ..., 2^(L-1)
    # γ(x): sin(2^k π x), cos(2^k π x) for k=0..L-1
    x_enc = jnp.concatenate([
        jnp.sin(freqs * jnp.pi * x[..., None]),
        jnp.cos(freqs * jnp.pi * x[..., None])
    ], axis=-1)  # (..., 2*L)
    y_enc = jnp.concatenate([
        jnp.sin(freqs * jnp.pi * y[..., None]),
        jnp.cos(freqs * jnp.pi * y[..., None])
    ], axis=-1)  # (..., 2*L)
    return jnp.concatenate([xy, x_enc, y_enc], axis=-1)

# ============================================================================
# SIREN: encoded (x, y) -> material in [0, 1] via sin(ω₀·(Wx+b)) hidden layers
# ============================================================================

def siren_init(key, layer_sizes, omega_0=30.0):
    """Initialize SIREN params."""
    params = []
...
def siren_apply(params, encoded_xy, omega_0=30.0):
    """Apply SIREN to encoded coords: hidden layers sin(ω₀·(Wx+b)), last layer linear then sigmoid."""
    h = encoded_xy
    for (W, b) in params[:-1]:
...
def siren_apply_batch(params, grid_points, omega_0=siren_omega0, num_freq=pos_enc_num_frequencies):
    """Encode (x,y), then apply SIREN batch."""
    encoded = positional_encode_2d(grid_points, num_freq)
    return vmap(lambda p: siren_apply(params, p, omega_0), in_axes=(0,))(encoded)

def material_field_from_mlp(params):
    """
    Sample SIREN on the grid with position-encoded (x,y) to get material α in [0,1].
    Material is allowed ONLY inside the subwindow; rest is water.
    """
...
def run_simulation_from_medium(medium, sim_time_axis=None):
    """Run j-wave simulation from a Medium."""
...
def compute_objective_mlp(params):
    """Objective: maximize RMS pressure over focal disk. Loss = -RMS."""
...
print("\n" + "="*70)
print("OPTIMIZATION: SIREN + POSITION ENCODING")
print("="*70)

learning_rate = float(os.environ.get('AUTODIFF_LR', '0.0001'))
n_iterations = int(os.environ.get('AUTODIFF_N_ITER', '100'))
checkpoint_path = os.path.join(results_dir, 'checkpoint_siren_subwindow_posenc.npz')
...
print(f"\nRunning full simulation for initial (random) SIREN...")
...
print(f"\nRunning full simulation for optimized SIREN...")
val_start = pytime.time()
...
print(f"\nFull simulation RMS: Initial {initial_max_pressure_full:.6f} Pa, Optimized {optimized_max_pressure_full:.6f} Pa, Improvement {improvement_full:.1f}%")
...
ax.set_title('SIREN + PosEnc Optimization Progress', fontsize=12, fontweight='bold')
ax.legend()
...
# Plot 3: Initial material field
ax = axes[0, 2]
alpha_init = np.array(material_field_from_mlp(initial_params)[0])
alpha_init = (alpha_init - c_water) / (c_cylinder - c_water)
...
# Plot 4: Optimized material field
ax = axes[0, 3]
...
ax.axis('off')
summary_text = f"""
SIREN + POSITION ENCODING SUBWINDOW SUMMARY

Pipeline:
  • (x,y) → positional encoding γ(x,y) → SIREN(γ) → α ∈ [0,1]
  • γ uses sin/cos at 2^0..2^{pos_enc_num_frequencies-1} π for x and y
  • α non-zero only inside subwindow; rest = water
  • Loss = -RMS(pressure in focal disk)

SIREN: {siren_layer_sizes}, ω₀={siren_omega0}
Pos enc: {pos_enc_num_frequencies} freq bands, input dim {siren_input_dim}
...
plt.suptitle('SIREN + Sinusoidal Position Encoding — Subwindow (GINN + J-Wave)',
            fontsize=14, fontweight='bold')
plt.tight_layout()

output_fig = os.path.join(results_dir, 'autodiff_siren_subwindow_posenc_results.png')
plt.savefig(output_fig, dpi=150, bbox_inches='tight')
print(f"Saved results to: {output_fig}")

print("\n" + "="*70)
print("SIREN + POSITION ENCODING OPTIMIZATION COMPLETE!")
print("="*70)
print("\nDone!")
