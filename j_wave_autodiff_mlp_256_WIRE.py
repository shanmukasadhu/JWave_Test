...
results_dir = os.path.join('Results/autodiff_mlp_256_WIRE', _run_id) if _run_id else 'Results/autodiff_mlp_256_WIRE'
os.makedirs('Results/autodiff_mlp_256_WIRE', exist_ok=True)
os.makedirs(results_dir, exist_ok=True)

print("="*70)
print("WIRE MATERIAL FIELD OPTIMIZATION (GINN + J-WAVE) — 256×256, FULL DOMAIN")
print("="*70)
print("\n256×256 grid, no viewing window. WIRE = Gabor wavelet activation.")
print("(x,y) -> material α in [0,1] (continuous blend water/solid).")
print("="*70)
...
# WIRE architecture: (x,y) -> [0,1] material. Gabor wavelet activation (Saragadam et al., CVPR 2023)
wire_layer_sizes = [2, 64, 64, 32, 1]
wire_sigma = 1.0   # Gabor Gaussian width
wire_omega = 30.0  # Gabor frequency
wire_seed = 42
...
print(f"  WIRE: {wire_layer_sizes}, σ={wire_sigma}, ω={wire_omega} (output α ∈ [0,1] continuous)")
print(f"  Objective: maximize RMS pressure over focal disk")
print("="*70)

# ============================================================================
# WIRE: (x, y) -> material in [0, 1] via Gabor wavelet φ(z)=exp(-z²/(2σ²))*cos(ωz)
# ============================================================================

def gabor_activation(z, sigma=1.0, omega=30.0):
    """Real part of complex Gabor wavelet: exp(-z²/(2σ²)) * cos(ωz). Element-wise."""
    return jnp.exp(-(z ** 2) / (2.0 * sigma ** 2)) * jnp.cos(omega * z)

def wire_init(key, layer_sizes, sigma=1.0, omega=30.0):
    """Initialize WIRE params. Pre-activation scaling suitable for Gabor (σ, ω)."""
    params = []
...
def wire_apply(params, xy, sigma=1.0, omega=30.0):
    """Apply WIRE: hidden layers Gabor(σ,ω)(Wx+b), last layer linear then sigmoid."""
    h = xy
    for (W, b) in params[:-1]:
        z = h @ W + b
        h = gabor_activation(z, sigma=sigma, omega=omega)
    Wlast, blast = params[-1]
    h = h @ Wlast + blast
    return jax.nn.sigmoid(jnp.squeeze(h))

def wire_apply_batch(params, grid_points, sigma=wire_sigma, omega=wire_omega):
    return vmap(lambda p: wire_apply(params, p, sigma=sigma, omega=omega), in_axes=(0,))(grid_points)

def material_field_from_mlp(params):
    """
    Sample WIRE on the grid to get material α in [0,1] (continuous).
    Design region only (x < design_boundary_x). Returns (sound_speed, density).
    """
    ...
    alpha = wire_apply_batch(params, grid_points)  # (N[0]*N[1],)
...
# ============================================================================
# WIRE init and Optimization Loop
# ============================================================================

key = jax.random.PRNGKey(wire_seed)
initial_params = wire_init(key, wire_layer_sizes, wire_sigma, wire_omega)

print("\n" + "="*70)
print("OPTIMIZATION: WIRE MATERIAL FIELD")
print("="*70)

learning_rate = float(os.environ.get('AUTODIFF_LR', '1.0'))
n_iterations = int(os.environ.get('AUTODIFF_N_ITER', '100'))
checkpoint_path = os.path.join(results_dir, 'checkpoint_wire_256.npz')
...
def arrays_to_params(arrs):
    ...
    params = []
    idx = 0
    for i in range(len(wire_layer_sizes) - 1):
        fan_in, fan_out = wire_layer_sizes[i], wire_layer_sizes[i + 1]
...
print("\nStarting optimization (gradient descent on WIRE parameters)...")
print("-"*70)
...
# Initial (random WIRE) medium and run
print(f"\nRunning full simulation for initial (random) WIRE...")
...
# Optimized WIRE medium and run
print(f"\nRunning full simulation for optimized WIRE...")
val_start = pytime.time()
...
ax.set_title('WIRE Optimization Progress (RMS over Focal Disk)', fontsize=12, fontweight='bold')
ax.legend()
...
ax.set_title('WIRE Gradient Magnitude', fontsize=12, fontweight='bold')
ax.grid(True, alpha=0.3)
ax.set_yscale('log')

# Plot 3: Learned material field (initial, random WIRE)
ax = axes[0, 2]
...
# Plot 4: Learned material field (optimized WIRE)
ax = axes[0, 3]
...
ax.axis('off')
summary_text = f"""
WIRE MATERIAL FIELD (GINN) SUMMARY

Pipeline:
  • WIRE(x,y) → α ∈ [0,1] (continuous)
  • α → sound_speed, density → jwave → pressure
  • Loss = -RMS(pressure in focal disk)
  • ∂Loss/∂(WIRE params) via JAX autodiff

WIRE: {wire_layer_sizes}, σ={wire_sigma}, ω={wire_omega}
...
plt.suptitle('WIRE Material Field Optimization — 256×256 Full Domain (GINN + J-Wave)',
            fontsize=14, fontweight='bold')
plt.tight_layout()

output_fig = os.path.join(results_dir, 'autodiff_wire_256_results.png')
plt.savefig(output_fig, dpi=150, bbox_inches='tight')
print(f"Saved results to: {output_fig}")

print("\n" + "="*70)
print("WIRE MATERIAL FIELD OPTIMIZATION COMPLETE!")
print("="*70)
print("\nLearned spatial material distribution via differentiable WIRE (Gabor) → jwave → RMS.")
print("="*70)
print("\nDone!")