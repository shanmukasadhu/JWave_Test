...
def plot_slices(
    ...
    # In-head-only planes (pressure masked to body, outside = black)
    has_in_head_row = body_mask is not None
    planes_in_head = None
    norm_in_head = None
    if has_in_head_row:
        p_abs_in_head = p_abs_disp.copy()
        p_abs_in_head[~body_mask] = np.nan
        planes_in_head = [p_abs_in_head[:, :, zmid].T, p_abs_in_head[:, ymid, :].T, p_abs_in_head[xmid, :, :].T]
    ...
    # Norm for in-head-only row
    if has_in_head_row and planes_in_head is not None:
        finite_in_head = p_abs_in_head[np.isfinite(p_abs_in_head)]
        vmax_in_head_row = float(np.percentile(finite_in_head, 99.5)) if finite_in_head.size > 0 else vmax
        vmax_in_head_row = max(vmax_in_head_row, 1e-12)
        vmin_log_in_head = max(1e-12, vmax_in_head_row / 1e5)
        norm_in_head = LogNorm(vmin=vmin_log_in_head, vmax=vmax_in_head_row)

    n_rows = 5 if has_in_head_row else 4
    fig, axes = plt.subplots(n_rows, 3, figsize=(18, 27 if has_in_head_row else 22))
    ...
    for i in range(3):
        ...
        _add_boundary_lines(axes[3, i], extents[i], i)
        _add_reference_point(axes[3, i], i)

    # --- Row 4: |P| in head only (outside = black) ---
    if has_in_head_row and planes_in_head is not None and norm_in_head is not None:
        for i in range(3):
            im = axes[4, i].imshow(
                np.where(planes_in_head[i] > 0, planes_in_head[i], np.nan),
                origin="lower", cmap=cmap_mag, norm=norm_in_head,
                extent=extents[i],
            )
            axes[4, i].set_title(f"|P| Pa (log) {names[i]}  (in head only)")
            axes[4, i].set_xlabel("mm"); axes[4, i].set_ylabel("mm")
            plt.colorbar(im, ax=axes[4, i], fraction=0.046, label="|P| (Pa)")
            _add_boundary_lines(axes[4, i], extents[i], i)
    ...
    plt.suptitle(
        f"Helmholtz CT-brain @ {p_ref:.2e} Pa peak  |  linear, full-domain, RMS, phase{mask_note}",
        fontsize=13,
    )
    plt.tight_layout()
