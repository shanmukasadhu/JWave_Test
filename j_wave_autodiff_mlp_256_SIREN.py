...
results_dir = os.path.join('Results/autodiff_mlp_256', _run_id) if _run_id else 'Results/autodiff_mlp_256'
os.makedirs('Results/autodiff_mlp_256', exist_ok=True)
os.makedirs(results_dir, exist_ok=True)

print("="*70)
print("SIREN MATERIAL FIELD OPTIMIZATION (GINN + J-WAVE) — 256×256, FULL DOMAIN")
print("="*70)
print("\n256×256 grid, no viewing window. Full domain = design + focus.")
...
def material_field_from_mlp(params):
    ...
    """
    # Normalized coords in [0, 1] so SIREN is grid-size independent
...
checkpoint_path = os.path.join(results_dir, 'checkpoint_mlp_256.npz')
...
# Initial (random SIREN) medium and run
...
# Optimized SIREN medium and run
...
ax.set_title('SIREN Optimization Progress (RMS over Focal Disk)', fontsize=12, fontweight='bold')
ax.legend()
...
plt.suptitle('SIREN Material Field Optimization — 256×256 Full Domain (GINN + J-Wave)',
            fontsize=14, fontweight='bold')
plt.tight_layout()

output_fig = os.path.join(results_dir, 'autodiff_mlp_256_results.png')
