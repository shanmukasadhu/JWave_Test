...
results_dir = os.path.join('Results/autodiff_mlp_256_SIREN_subwindow', _run_id) if _run_id else 'Results/autodiff_mlp_256_SIREN_subwindow'
os.makedirs('Results/autodiff_mlp_256_SIREN_subwindow', exist_ok=True)
os.makedirs(results_dir, exist_ok=True)

print("="*70)
print("SIREN MATERIAL FIELD — SUBWINDOW (75% OF DESIGN SPACE)")
print("="*70)
print("\n256×256 grid. Material is learned ONLY inside a subwindow (75% of design space).")
...
target_focal_center = (160, 128) # Slightly upper left point: (190, 240)
...
checkpoint_path = os.path.join(results_dir, 'checkpoint_siren_subwindow.npz')
...
ax.axis('off')
summary_text = f"""
SIREN SUBWINDOW (75% DESIGN) SUMMARY

Pipeline:
  • SIREN(x,y) → α ∈ [0,1] (continuous)
  • α non-zero only inside subwindow (75% of design); rest = water
...
plt.suptitle('SIREN Material Field — Subwindow 75% of Design (GINN + J-Wave)',
            fontsize=14, fontweight='bold')
plt.tight_layout()

output_fig = os.path.join(results_dir, 'autodiff_siren_subwindow_results.png')
